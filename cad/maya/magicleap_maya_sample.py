...
###########################################################################
#
#  Start of Magic Leap configuration
#
#  This file is compatible with Magicleap SDk version 0.24.0+.
#
#  Tested with the following combination:
#   Mac OS Version 10.15.1 with Maya 2018
#   Windows Version 10 with Maya 2019
#   DCA version 96 OSX
#
#   WonderMediaProduction commit 40b77bc6d5b2d2e5a477a247cbf8f074a75dcca5
#   WonderMediaProduction only works in Maya 2018
#
#   matiascodesal/maya-glTF commit 4009b9b5a030662314e14a22016740e492158385
###########################################################################
...
#
# glbpath: glb file temporary storage on desktop/laptop
# dcafile: Magic Leap's Desktop Companion App binary execution file in full path
#
# Note:
...
if platform.system() == 'Darwin': # Mac
	glbpath = "/var/tmp/magicleap/glb"
	dcafile = "/Users/Shared/Autodesk/maya/plug-ins/dcacla"
elif platform.system() == 'Windows': # Windows
	glbpath = "c:\\temp\\magicleap\\glb"
	dcafile = "c:\\Progra~1\\Autodesk\\Maya{}\\plug-ins\\dcacla.exe".format(mayaVersion)

print ""
print "###########################################################################"
print "# Welcome to Magic Leap sample Maya plugin"
print "#"
print "# Description:"
print "#   This plug make use of open source to generate glb file and then send to"
print "#   Magic Leap devices for MR style preview"
print "###########################################################################"
print ""
print "Current running OS: {}, Maya version {}".format(os.name, mayaVersion)
print ""
print "glbpath = '{}'".format(glbpath)
print "dcafile = '{}'".format(dcafile)

###########################################################################
# End of Magic Leap configuration
###########################################################################
...
##############################################################################
#   Start of matiascodesal/maya-glTF Open Source with MIT license
#
#   matiascodesal/maya-glTF Open Source with MIT license
#
#   https://github.com/matiascodesal/maya-glTF/blob/master/scripts/glTFExport.py
#   commit 8a6ed49ef456f5d6907e35a4bd03210647b80f0f
#
##############################################################################
...
# matiascodesal/maya-glTF don't export hidden nodes
def timeit(method):
...
# matiascodesal/maya-glTF class for Resource Formats
class ResourceFormats(object):
	EMBEDDED = 'embedded'
	SOURCE = 'source'
	BIN = 'bin'

# matiascodesal/maya-glTF class for Animation Option
class AnimOptions(object):
	NONE = 'none'
	KEYED = 'keyed'
	BAKED = 'baked'

# matiascodesal/maya-glTF class for Property
...
# matiascodesal/maya-glTF method for class property
def classproperty(func):
	if not isinstance(func, (classmethod, staticmethod)):
		func = classmethod(func)
	return ClassPropertyDescriptor(func)

# matiascodesal/maya-glTF class for Setting up the export feature
...
# matiascodesal/maya-glTF the main class for glb export
class GLTFExporter(object):
	...
	# matiascodesal/maya-glTF entry point to run glb export
	def run(self):
		...
		if not os.path.exists(ExportSettings.out_dir):
			print (ExportSettings.out_dir)
			os.makedirs(ExportSettings.out_dir)

		scene = Scene()
		# matiascodesal/maya-glTF only support exporting single scenes,
...
# matiascodesal/maya-glTF helper method to export glb
...
# matiascodesal/maya-glTF class for encoding into glb
...
# matiascodesal/maya-glTF base class
class ExportItem(object):
	def __init__(self, name=None):
		self.name = name

# matiascodesal/maya-glTF class for Scene, see GLTF spec for detail
...
# matiascodesal/maya-glTF class for describe the Node, see GLTF spec for detail
class Node(ExportItem):
	...
	def _get_rotation_quaternion(self):
		...
		if (obj.hasFn(OpenMaya.MFn.kTransform)):
			...
			#then matiascodesal/maya-glTF can add it to transfrom Fn
...
# matiascodesal/maya-glTF class for describe the Mesh, see GLTF spec for detail
class Mesh(ExportItem):
	...
	def _getMaterial(self):
		...
		# matiascodesal/maya-glTF currently only support one materical per mesh, so it just grab the first one.
	...
	def _getMeshData(self):
		...
		while not meshIt.isDone():
			...
			for point, vertex_index in zip(points, ids):
				...
				# but matiascodesal/maya-glTF really don't know what udim it's in for that case.
...
# matiascodesal/maya-glTF class for describe the Material, see GLTF spec for detail
class Material(ExportItem):
	...
	def to_json(self):
		...
		# matiascodesal/maya-glTF don't Support doubleSided property
...
# matiascodesal/maya-glTF class for describe the Camera, see GLTF spec for detail
...
# matiascodesal/maya-glTF class for describe the Camera
...
# matiascodesal/maya-glTF class for describe the Animation, see GLTF spec for detail
...
# matiascodesal/maya-glTF class for describe the Animation Channel
...
# matiascodesal/maya-glTF class for describe the Animation Sampler
...
# matiascodesal/maya-glTF class for describe the Images
...
# matiascodesal/maya-glTF class for describe the Texture
...
# matiascodesal/maya-glTF class for describe the Sampler
class Sampler(ExportItem):
	def __init__(name=None):
		super(Sampler, self).__init__(name=name)


# matiascodesal/maya-glTF class for describe the Buffer, see GLTF spec for detail
...
# matiascodesal/maya-glTF class for describe the BufferView, see GLTF spec for detail
...
# matiascodesal/maya-glTF class for describe the Types, see GLTF spec for detail
class ComponentTypes(object):
	USHORT = 5123
	UINT = 5125
	FLOAT = 5126

# matiascodesal/maya-glTF class for describe the Accessor, see GLTF spec for detail
...
##############################################################################
# end of matiascodesal/maya-glTF Open Source with MIT license
##############################################################################
...
##############################################################################
# start of Magic Leap code
##############################################################################

import maya.cmds as cmds
import subprocess
import maya.api.OpenMaya as OpenMayaApi2
...
#
# Check the plug is available or not by the plugin name
#
def checkPlugin(name):
	try:
		if cmds.pluginInfo( name + '.py', query=True, command=True ) == None:
			return False
		return True
...
#
# Magic Leap Plugin make use of open source GLB exporters
# Maya 2018 doesn't build-in with any glb export
#
# 1. Wonder Media Productions
#   Note: it requires end user manually download, build, and install
#         works only in Maya 2018 or earlier supported version by this vendor
#   visit https://github.com/WonderMediaProductions/Maya2glTF for installation
#   MIT lience https://github.com/WonderMediaProductions/Maya2glTF/blob/master/LICENSE
#
# 2. matiascodesal
#   Note: open source code has been copy-pasted on top of this file
#   visit https://github.com/matiascodesal/maya-glTF/blob/master/scripts/glTFExport.py
#   MIT lience https://github.com/matiascodesal/maya-glTF/blob/master/LICENSE
#
def exportGLB(glbpath, filename):
	fullfilepath = os.path.join(glbpath, filename + ".glb")

	if checkPlugin("maya2glTF") == True and cmds.about(version=True) == "2018":
		print("\nPacking the scene with Wonder Media Productions to {} ...".format(fullfilepath))
		cmds.maya2glTF(sceneName=filename, # no file extension
				glb=True,
				outputFolder=glbpath,
				scaleFactor=1,
				cpr="(C)2019 MagicLeap Inc.")
	else:
		print("\nPacking the scene with matiascodesal to {} ...".format(fullfilepath))
		export(file_path=fullfilepath,
				resource_format='bin',
				anim='keyed',
				vflip=True)

#
# Magic Leap make use of Desktop Companion Application (DCA) to send file to Magic Leap Devices
#
# dcacla is the binary executable that this script is using
#
def checkDCAExist():
	print("checking dca command line...")
	ret = os.path.exists(dcafile)
	if  ret == False:
		print("{} not found!!".format(dcafile))
		cmds.error("Not found! {}".format(dcafile))
	else:
		print("DCA found! {}".format(dcafile))

	return ret


# Send file using DCA command line to Magic Leap device
#
# usage:
#   To push file into the user document folder:
# 		dcacla push -D srcfile.glb dstfile.glb
#
#   To push file into a sandbox folder of an application:
# 		dcacla push -pcom.magicleap.gallery srcfile.glb dstfile.glb
#
def DCASendFile(glbpath, filename):
	fullfilename = filename + ".glb"
	fullfilepath = os.path.join(glbpath, fullfilename)
	print("\nSending {} to magicleap device...".format(fullfilepath))

	# "-D"   goes to user documents folder
	cmd = [dcafile, "push", "-D", "\"{}\"".format(fullfilepath) , "\"{}\"".format(fullfilename)]
	cmd = " ".join(cmd)
	print("Executing: {}".format(cmd))

	# Note:
	#   Maya Python2.7 subprocess doesn't work well in other ways
	#   making it shell=True & PIPE may be the most robust way
	#   tested with Maya 2018 + Python 2.7
	...
	print("last modified: {}".format(datetime.datetime.fromtimestamp(os.stat(fullfilepath).st_ctime)))
	return (res, err)


#
# The Main class for MagicLeap process
#
class MagicLeapMain():
	menu = None # The menu instance for uninitialization

	#
	# Magic Leap main runner method
	#   Send the current selected object from Maya to Magicleap device
	#
	@staticmethod
	def run(args=None):
		print("")
		print("#####################################")
		print("#  Running Magic Leap Main")
		print("#  args: {}\n".format(args))
		checkDCAExist()

		# simply use the scene name as the file name, e.g. myScene.mb
		filename = "{}".format(cmds.file(q=True,sn=True,shn=True) or "untitled")

		exportGLB(glbpath, filename)
		(res, err) = DCASendFile(glbpath, filename)

		if len(res) > 0:
			print("DCA result = '{}'".format(res))
			cmds.inViewMessage( amg='<br>Desktop Companion:</br> <hl> {} </hl>.'.format(res), pos='midCenter', fade=True )
			maya.OpenMaya.MGlobal.displayInfo("DCA result: '{}'".format(res))

		if len(err) > 0:
			print("DCA error = '{}'".format(err))
			cmds.inViewMessage( amg='<br>Desktop Companion:</br> <hl> {} </hl>.'.format(err), pos='midCenter', fade=True )
			cmds.error("DCA Error!!! {}".format(err))
		print("\n#####################################")

#
# Maya standard plugin entry method
#
def initializePlugin( mobject ):
	...
	# Build a menu under the Maya Window
	mlMenu = pm.menu('Magic Leap', parent=MainMayaWindow)
	# Add a 'Export' menu item under Magic Leap
	pm.menuItem(label="Export", command=MagicLeapMain.run, parent=mlMenu)
	print("'{}' menu created via pm".format(mlMenu))
	MagicLeapMain.menu = mlMenu


#
# Maya standard plugin exit method
#
def uninitializePlugin( mobject ):
	print("[uninitializePlugin]")
	menuId = MagicLeapMain.menu
	if cmds.menu(menuId, q=True, exists=True):
		print("'{}' menu removed via cmds".format(menuId))
		cmds.deleteUI(menuId, menu=True)
		MagicLeapMain.menu = None
	elif pm.menu(menuId, exists=True):
		print("'{}' menu removed via pm".format(menuId))
		pm.deleteUI(menuId)
		MagicLeapMain.menu = None


# The convinence call for developers using Maya's script editor
